...
class DocumentSearch extends Component {
    ...
    handleDocumentSearch() {
        ...
        if (this.state.similaritySearch) {
            this.similarityService.getSimilarities(this.state.search_term).then((similarities) => {

                let results = [];

                similarities.data.datasets.forEach((hit) => {
                    console.log(hit.label);
                    results.push(hit.label);
                });
            ...
            <div>
                    ...
                    <FormControl className={classes.formControl}>
                        <Select
                            ...
                            renderValue={selected => selected.length === 0 ?
                                <TagsPlaceholder>Tags</TagsPlaceholder> : selected.join(', ')}>
                ...
                <Results>
                    <ResultsList data={this.state.results} handleDocumentDelete={this.handleDocumentDelete}
                                 history={this.props.history} similarity={this.state.similaritySearch}/>
                </Results>
                }
            </div>
        )
